...
Progress is `tracing`, never stdout: the engine emits a handful of INFO events at its slow steps (each source extraction, each synthesis pass) and DEBUG detail (claim counts), rendered by the guest subscriber `execute_wasi` installs and filtered by the guest's `RUST_LOG`. The semantic result stays the buffered `Response`; no engine code writes a process stream.
