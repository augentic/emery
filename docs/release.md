...
The host embeds no adapter-version recommendation: exact package pins arrive as run input, fetched through the compiled-in `locations:` policy (the `omnia.host` default endpoint, with [`src/wasm-pkg.toml`](../src/wasm-pkg.toml) routing namespaces to their registries; nothing in a project overrides it), and local components load by path (the journey host in [`examples/runtime.rs`](../examples/runtime.rs) loads its built mock source that way); the shipped runtime embeds the engine only. Statically declared adapter guests remain possible in a custom runtime invocation.
