...
- Stream roles: the semantic result body (text or JSON) is **stdout**; the failure envelope and live tracing are **stderr**. Tracing follows the process `RUST_LOG`, which the shipped runtime defaults to `emery_sdk=info` when unset (see [cli-contract.md](../standards/cli-contract.md)).
