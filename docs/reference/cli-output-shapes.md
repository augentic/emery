...
- Stream roles: the semantic result body (text or JSON) is **stdout**; the failure envelope and live tracing are **stderr**. Tracing verbosity is selected by the global `-v` / `--verbose` and `-q` / `--quiet` flags of the `emery` grammar (see [cli-contract.md](../standards/cli-contract.md)).
