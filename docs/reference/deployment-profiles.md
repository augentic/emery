...
The `spec.md` / `design.md` projections a project keeps beside its code are not engine state either: they are `emery show` output the skill writes into the working tree for review, and nothing reads them back. Loaded components are not engine state: every adapter loads through the deployment's `omnia:plugins/loader` capability at the location the engine names, inside the grant the shipped runtime fixes at compile time. A local component is read fresh on every run through the read-only project mount, at the project-relative path the reference names — the one root code is read through, and a mount a guest could write is never one; a package is fetched fresh on every run from the registry the engine names on the load — the project's `emery.toml` `[registries]` line, or `augentic.io` for `emery` — the shipped profile compiling in no routing and no project cache, so no fetched component persists between runs and resolution is always fresh-release-preferred. Which registry a package is fetched from is therefore the project `emery.toml`'s decision — the same operator-owned file that can name a local component outright — and the binary compiles in no allow-list of registries; an operator who must bound registry egress bounds what that file may say. Earlier cache trees (`.omnia/cache/wasm-pkg`, `.omnia/storage/plugins/`) are orphaned — nothing reads them; delete them freely. Integrity binds the resolved sha256 digest to the exact bytes the host executes — a `[[source]] digest` pins it — never an engine-owned mutable sidecar. A bare name loads as a guest the deployment declares, attested rather than fetched, and implies no stored component.
