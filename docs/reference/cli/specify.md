...
A run continues a revision rather than starting over. A project that carries `.emery/spec.json` and `.emery/design.json` — the [`emery show --format json`](show.md) envelopes of one revision, written beside the code after a run — hands that revision in: it is discovered on every `specify` like the project-root `emery.toml` (no flag), adopted as the current revision (displacing whatever the store held), and the run's requirements inherit its ids — through any cited `(source, claim)` pair, else through an equal subject; several continued requirements merge onto the lowest id, a split keeps the id on the requirement holding the subject claim, and new requirements allocate from the revision's `next_id`, so an id is never reused. A requirement whose facts stand (subject, status, coverage, sources, body, losers) keeps its scenarios without a model turn; a run in which every requirement stands skips the spec turn, and a design that still verifies over an unchanged specification is kept whole. With nothing carried, the store's current revision anchors the run the same way; an outdated or unreadable current anchors nothing, and the run regenerates.

The pair travels together and is read whole: one file without the other, a file that is not a `show --format json` envelope, or an envelope whose `document` is not a revision fails typed with `revision-invalid` (exit `1`) before any adapter loads; a carried revision stamped with an older grammar fails with `spec-outdated` (exit `1`). Both hints name the way out — restore the pair from one revision, or remove `.emery/` to regenerate.
...
- `diff` — the re-mine diff against the revision the run continued (the adopted `.emery/` revision, else the outgoing current): `from`, then a `{ added, removed, changed }` object each for `spec` (requirements matched by `id`, as `{ id, subject }`, a `changed` entry naming the differing `fields`) and `design` (section keys); absent on a first run, every list empty on a byte-stable re-run (see [CLI output shapes](../cli-output-shapes.md#emery-specify))
