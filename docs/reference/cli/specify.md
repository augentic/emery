...
Generate the specification and design masters from the sources named on the invocation and commit them as one revision.
...
Each run resolves its adapters before extracting; a local `.wasm` component or an exact package reference loads through the deployment's `omnia:plugins/loader` capability. A component is read fresh on every run — nothing is mirrored, so deleting the file makes the next run refuse typed; a package fetches from the source's `registry` override or the compiled-in default endpoint (`omnia.host`), likewise read fresh on every run — the deployment keeps no project cache. A source's optional `digest` pin is verified host-side before validation. Extract dispatches every source over the `Source` capability; the engine then groups the requirement claims into requirements (byte-equal ids always merge; across two or more sources the model judges the rest, with authority withheld), ranks each requirement's agreeing classes under authority precedence (intent > documentation > behaviour) into its status, drafts the scenarios and design content through schema-gated model answers checked against those requirements and the claim-derived section plan (each requirement's body is the winning claim's statement, rendered by the engine), places the accepted drafts beside its facts in the typed masters, and commits them as one revision, atomically swapping the current revision id. Gaps stay `[unknown]`; disagreement surfaces inline as `[conflict]` / `[divergence]`. Re-running over identical sources is byte-stable and reports an empty re-mine diff in the success envelope — nothing is persisted for the diff. Review the committed set with [`emery show`](show.md).

### The carried master

A run continues a revision rather than starting over. A project that carries `.emery/spec.json` and `.emery/design.json` — the [`emery show --format json`](show.md) envelopes of one revision, written beside the code after a run — hands that master in: it is discovered on every `specify` like the project-root `emery.toml` (no flag), adopted as the current revision (displacing whatever the store held), and the run's requirements inherit its ids — through any cited `(source, claim)` pair, else through an equal subject; several continued requirements merge onto the lowest id, a split keeps the id on the requirement holding the subject claim, and new requirements allocate from the master's `next_id`, so an id is never reused. A requirement whose facts stand (subject, status, coverage, sources, body, losers) keeps its scenarios without a model turn; a run in which every requirement stands skips the spec turn, and a design that still verifies over an unchanged specification is kept whole. With nothing carried, the store's current revision anchors the run the same way; an outdated or unreadable current anchors nothing, and the run regenerates.

The pair travels together and is read whole: one file without the other, a file that is not a `show --format json` envelope, or an envelope whose `document` is not a master fails typed with `master-invalid` (exit `1`) before any adapter loads; a carried master stamped with an older grammar fails with `spec-outdated` (exit `1`). Both hints name the way out — restore the pair from one revision, or remove `.emery/` to regenerate.
...
- `diff` — the re-mine diff against the revision the run continued (the adopted `.emery/` master, else the outgoing current): `from`, then a `{ added, removed, changed }` object each for `spec` (requirements matched by `id`, as `{ id, subject }`, a `changed` entry naming the differing `fields`) and `design` (section keys); absent on a first run, every list empty on a byte-stable re-run (see [CLI output shapes](../cli-output-shapes.md#emery-specify))
