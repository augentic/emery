...
- **`I`** is a flat, transport-neutral serde DTO, `<Verb>Input` (`Serialize`/`Deserialize`, `#[serde(rename_all = "kebab-case")]`): `SpecifyInput { sources: Vec<SourceConfig>, carried: Option<Carried> }`, `ShowInput { document: Document }`. It carries no clap derives, no flag names, and no carrier knowledge — the same type deserializes from an HTTP body (`omnia_guest::api::http::post(specify)`) as is built by the CLI façade.
...
The grammar lives on façade-side `SpecifyArgs` / `ShowArgs` (`clap::Args`, `#[arg]` parsers, `--help` prose). Each decodes into its engine input by **exhaustive struct literal** (`SpecifyInput { sources, carried }`, `ShowInput { document }`), so a new engine field is a façade compile error — the same drift guarantee the old fused design had, with one direction of dependency. Closed engine vocabularies are not mirrored as clap enums: `show`'s `document` is the engine's `Document` itself, parsed through a `PossibleValuesParser` built from its `strum` keys (`Document::VARIANTS`, `as_ref()`, `parse()`), with the per-value help line an exhaustive `match` in the façade — so a new variant is likewise a compile error, and no clap derive reaches the engine. Global flags (`--format`) stay on `App`. Layering rule: `cli` imports engine inputs, outputs, the source and carried DTOs (`SourceConfig`, `Carried`), `Document`, `AdapterRef`, `preopen_path`, and `omnia_guest::Error` — never domain kernels.
