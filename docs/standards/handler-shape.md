...
On wasm, the guest (`src/lib.rs`) explicitly exports `wasi:cli/run` and hands `dispatch` to `execute_wasi`, the sole owner of telemetry initialization, flushing, channel writes, and process exit. `dispatch` runs `emery_cli::run` over its provider and reloads the guest tracing filter to the reported verbosity through `omnia_wasi_otel::set_filter`; the CLI opens its `command` span after that callback. The returned `Response` implements `IntoExit`, so `execute_wasi` writes both channels (a `BrokenPipe` keeps the response's own exit, any other refused channel exits 3), flushes the closed span, and exits with the exact status. Every path runs the same grammar and projector.
