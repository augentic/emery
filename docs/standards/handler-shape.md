...
On wasm, the guest (`src/lib.rs`) exports `wasi:cli/run` and hands `dispatch` to `execute_wasi`, the sole owner of telemetry initialization, flushing, channel writes, and process exit — the body of `omnia_sdk::command!(dispatch)`, written out so the export reads in place. `dispatch` runs `emery_cli::run` over its provider; the verbosity callback hands the level's preset to `omnia_wasi_otel::set_filter`, which applies the process `RUST_LOG` after it, then names the level on the dispatch chain so every adapter the run dispatches opens its own tracing at it. `Client::call` then opens the `handler` span carrying invocation metadata. The returned `Response` implements `IntoExit`, so `execute_wasi` writes both channels (a `BrokenPipe` keeps the response's own exit, any other refused channel exits 3), flushes guest telemetry, and exits with the exact status. Every path runs the same grammar and projector.
