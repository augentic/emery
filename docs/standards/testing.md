...
There is **no wasm rung** in this repository. The root build script compiles the engine guest (`emery-cli` over `emery-engine` and `emery-adapter`) to `wasm32-wasip2` on every native build; the adapter SDK's export side and the mock adapter are compiled for wasm32 only by the live journey's `cargo build --example adapter --target wasm32-wasip2 --release`. Instantiating the `emery:adapter/source` seam under the real omnia runtime — the WIT lowering on both sides, the reference-tool closure over real wasi-model tool streams — is owned by `emery-adapters`' conformance rung, which drives every first-party component through the published contract.
