...
- `tests/specify.rs` — the `specify` → `show` product arc: sources (argv, `--description`, `--config`, root discovery), extraction and the extras gate, the up-to-three typed model turns and their gates (the grouping partition over the id baseline, the `spec.md` draft against the requirements, the `design.md` draft against the section plan — each refused typed once the backend's rounds are spent, with one corrected-draft scenario asserting the tightened schema rides the request and the findings ride the recorded `check` correction), the canonical masters asserted byte-for-byte against the stored `spec.json` / `design.json` (`tests/specify/<n>-*.json`) and the Markdown projections against `emery show` stdout (`tests/specify/<n>-*.md`, their `<revision>` placeholder filled from the store), the master a run continues (the carried `.emery/` pair adopted as current, `master-invalid` / `spec-outdated` refusals, ids inherited through cited `(source, claim)` pairs then subject — a merge keeping the lowest id, `next_id` never reused — and the incumbent rule: a requirement whose facts stand keeps its scenarios unasked, a standing spec keeps a still-verifying design unasked), the revision store (commit, pruning, corruption), typed re-mine diffs keyed by id, and multi-project isolation.
...
- Prefer structural assertions (status fields, exit codes, JSON shape) over byte-for-byte prose comparisons, except where bytes are the contract (the stored master is canonical JSON; `show` renders the projection alone).
