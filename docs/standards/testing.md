...
There is **no wasm rung** in this repository. The root build script compiles the engine guest (`emery-cli` over `emery-engine` and `emery-adapter`) to `wasm32-wasip2` on every native build; the adapter SDK's export side and the mock adapter are compiled for wasm32 by `make lint`'s second clippy pass (`--target wasm32-wasip2` over `emery-sdk`, the engine guest, and the `adapter` example — a `cfg(target_arch = "wasm32")` defect fails `make ci` there, not at the live journey), and built as a component only by the live journey's `cargo build --example adapter --target wasm32-wasip2 --release`. Instantiating a component over `emery:adapter/source` under the real omnia runtime — the WIT lowering on both sides, the reference-tool closure over real wasi-model tool streams — is owned by `emery-adapters`' conformance rung, which drives every first-party component through the published contract.

Engine state is observed **through the storage provider and the success envelope, not the filesystem**: since the storage capabilities (design/portable-storage.md steps 1–2), engine-owned state (the revision store) is written through the `omnia_sdk::StateStore`/`BlobStore` capabilities — the `wasi:keyvalue`/`wasi:blobstore` imports in deployment, bound host-side to the durable `omnia-filesystem` store — and the native suites script them with omnia-test's `guest::{Memory, Namespaced}`; every adapter load — a local component by path, a package at its routed registry, a bare name attested as a declared guest — is scripted through the support module's `Plugins` double, recording the `Location` and pin of each load and resolving it to a scripted digest, so a suite asserts what the engine asked the loader for (`provider.plugins.loads()`) and never what a host did with it. No suite changes the process working directory — except the config-discovery scenarios, which enter a `tempfile::TempDir` to control what a run naming no sources finds, or what `[registries]` table an argv run routes through (safe because nextest runs each test in its own process) — and no suite asserts an on-disk layout: the backing is host policy, exercised on the live rung, not in tests. Operator-supplied inputs (a local `.wasm` component, an `emery.toml`) stay real files in a `tempfile::TempDir`.

### The mock adapter

The `adapter` example (`examples/adapter/`, a root-package `[[example]]` cdylib) is the one mock source adapter: a guest of the `source-adapter` world whose `extract` hands its survey's seam to `emery_sdk::extract` over the host `Model` and its fixture tree, with its embedded prose under `examples/adapter/prose/`. It is the live-journey fixture (the shipped binary hosts the built component through `examples/emery.toml`, loading it by path from the `.` mount). Do not add another mock adapter, mock model, or mock-adapter copy — extend this example. The root suites' scripted `Source` (`tests/support/provider.rs`) is a capability double, not an adapter: it never parses a workspace and carries no extraction behavior beyond the scenario's scripted evidence.

Capability doubles come from omnia's `omnia-test` crate (a native-only dev-dependency): `guest::Scripted`, the FIFO request-recording `Model` the root suites script directly, `guest::ScriptedLoader`, the `Plugins` loader keyed by the name a `Location` derives (admitting every component path and package it is asked for unless a refusal is scripted, as the shipped runtime's read-only project mount and registry routing admit them, and a bare name only where the scenario declared it through `Provider::declaring([..])` — every other refused as the shipped runtime, which declares no adapter, refuses every one, so each scenario shows the deployment it assumes; recording every `(Location, Option<Digest>)` it was asked for, so a suite asserts the arm and the pin the engine put on the load; seeded with `defaulting(digest("ab"))`, the fixed digest an unscripted load resolves to, so a `[[source]] digest` scenario scripts the pin per name and a pin the resolved digest does not match is refused as the real loader refuses it), and the storage pair `guest::{Memory, Namespaced}`; The shared `Provider` implements each capability trait by forwarding to the field holding its double — plain impls, one fully qualified call each. `Scripted` is strict: running past the script panics, and dropping a script with unconsumed turns fails the test — a scenario scripts exactly what its run consumes (a refusal before the model is reached scripts no answers; a transport failure is scripted as `Err(..)`, not as an empty script). Scenario doubles — the scripted `Source` and the shared provider — live in the root package's `tests/support/provider.rs`, which every root suite includes; `tests/support/mod.rs` adds the response assertions the product suites share — `fail` also snapshots the scripted storage around the refused run and asserts it came back byte-equal, so no scenario re-asserts that a refusal commits nothing. Suites own only scenario content and assertions.
...
```bash
make cov                                              # workspace-wide: cargo llvm-cov nextest --workspace --all-features --summary-only
cargo llvm-cov nextest -p emery-<crate> --summary-only   # focused: one leaf crate's own contract suite
```
