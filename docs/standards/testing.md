...
The `adapter` example (`examples/adapter/`, a root-package `[[example]]` cdylib) is the one mock source adapter: a `emery_sdk::SourceAdapter` implementor whose extract calls the host `Model` over its fixture tree, with its embedded prose under `examples/adapter/prose/`. It is the live-journey fixture (the `runtime` example hosts the built component with the Cursor backend and a path-only plugin `locations:` list). Do not add another mock adapter, mock model, or mock-adapter copy — extend this example. The root suites' scripted `Source` (`tests/support/provider.rs`) is a capability double, not an adapter: it never parses a workspace and carries no extraction behavior beyond the scenario's scripted evidence.
