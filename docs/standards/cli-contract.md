...
| 1    | `BadRequest`   | Operator or input refusal. The `error` field is `specify-source-required`, `unsupported-version`, `spec-outdated`, the loader's refusal (`refused`), or the Omnia default `bad_request`. |
