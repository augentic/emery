...
Stream roles are part of the contract: the semantic result body (text or JSON) is stdout; the failure body and live tracing are stderr. The failure body carries no styling in either format; colour policy belongs to the tracing layer alone. One tracing level governs a run — the runtime's host console, the engine, and every adapter — and the global `-v` / `--verbose` and `-q` / `--quiet` flags select it. They are omnia's: the runtime reads them from argv and sets every guest's `RUST_LOG` before the engine runs, and the grammar declares them (`omnia_sdk::api::command::Verbosity`, flattened) so help and completions list them and `-v` beside `-q` is a usage error; the engine never reads their values. A bare run is `info`: the engine's progress at its slow steps, the SDK's report of each model turn as it opens, and a turn retried or failed at WARN. Each `-v` raises the level one step and each `-q` lowers it, clamped: `-q` warn, `-qq` error, `-qqq` off; `-v` debug (the engine's and the SDK's detail — what each turn yielded, each reference-tool call, each rejected candidate), `-vv` trace. A flag replaces a process `RUST_LOG`; a bare run keeps one that is set, so `RUST_LOG=emery_sdk=debug emery specify …` admits the SDK's detail alone and `RUST_LOG=off` silences every guest. The host never rewrites the process environment. A failure's description is printed once, in the result body; tracing names the failed step and its class, never the text. Skills follow the plugin rule's tracing contract and relay the semantic result once without repeating tracing lines.
...
- `refused` — the loader rejected the load: an invalid component, a missing `emery:adapter/source` export, a pre-compiled artifact where the entry admits raw wasm alone (every adapter the shipped runtime declares), a mismatched digest pin, or a bare name this deployment does not declare; the message names which.
...
| 1    | `BadRequest`   | Operator or input refusal. The `error` field is `specify-source-required`, `unsupported-version`, `spec-outdated`, a loader refusal (`refused`, `already-active`), or the Omnia default `bad_request`. |
| 2    | `NotFound`     | Missing resource. The `error` field is `spec-not-generated` or the Omnia default `not_found`.                                                                                                                                                  |
| 3    | `ServerError`  | Evidence the claim gate rejects, or unclassified default: I/O, storage, leftover conversions. The `error` field is the Omnia default `server_error` or the loader's `internal`.                                                                 |
