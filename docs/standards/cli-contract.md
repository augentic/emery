...
Stream roles are part of the contract: the semantic result body (text or JSON) is stdout; the failure body and live tracing are stderr. The failure body carries no styling in either format; colour policy belongs to the tracing layer alone. Tracing verbosity is selected by the global flags `-v` / `--verbose` (counted) and `-q` / `--quiet`, part of the `emery` grammar and accepted anywhere in argv: a bare invocation shows INFO progress, `-v` adds the engine's DEBUG detail, `-vv` adds its TRACE detail and every engine-guest crate's DEBUG, and `-q` turns the engine's tracing off. An ambient `RUST_LOG` refines every level, `-q` included — its valid directives apply after the flag's, so for a target both name the environment's wins and a bare level in it replaces the flag's; an invalid directive is diagnosed on stderr and ignored. Combining `-v` with `-q` is a usage error (exit `64`). The flags govern the engine alone. An adapter is its own guest and follows its guest environment's `RUST_LOG`, which the shipped runtime defaults to `emery_sdk=info` when the process sets none: on any run the SDK reports each model turn as it opens and a turn retried or failed at WARN, and an operator's own `RUST_LOG` replaces the default whole (`RUST_LOG=emery_sdk=debug emery specify …` adds what each turn yielded, each reference-tool call, and each rejected candidate; `RUST_LOG=off` silences the adapters). A fully silent run is `RUST_LOG=off emery -q …`. The host never rewrites the process environment. A failure's description is printed once, in the result body; tracing names the failed step and its class, never the text. Skills follow the plugin rule's tracing contract and relay the semantic result once without repeating tracing lines.
