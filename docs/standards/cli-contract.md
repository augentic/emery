...
- `emery show <spec|design>` — print a reviewable document of the current revision; text stdout is the Markdown projection alone, and the JSON envelope carries the typed master as `document`. Before any commit it fails `spec-not-generated` (exit `2`); a master under an older grammar fails `spec-outdated` (exit `1`).
- `emery completions <shell>` — auto-derived shell completions over the live clap surface.

## JSON envelope

Every CLI verb that skills consume emits a stable **flat body**: the command-specific fields at the top level of a single JSON object. On success the body is exactly that — there is no `ok` discriminant, no `data` wrapper around the payload, and no top-level envelope-version stamp. On failure the flat object carries three top-level keys: `error` (a discriminant string: kebab-case for the five recovery codes, snake_case for Omnia defaults), `message` (a humanised one-liner), and `exit-code` (the integer the binary returns). Skills invoked with `--format json` parse the body and branch on the `error` field rather than on stdout text.
...
- `spec-outdated` — the master (stored, or carried in `.emery/`) was written under an older grammar than the running binary reads; regenerate.
- `master-invalid` — the carried `.emery/spec.json` / `.emery/design.json` pair is incomplete, not `emery show --format json` envelopes, or not a master; restore both from one revision or remove `.emery/`.
...
Skills should branch on the exit code first (success vs failure class) and on the five kebab recovery discriminants second (`specify-source-required`, `unsupported-version`, `spec-not-generated`, `spec-outdated`, `master-invalid`). Other failures share the Omnia snake_case default for that class (`bad_request`, `not_found`, `server_error`, `bad_gateway`). New exit codes are not invented by skills or the CLI.
