...
- `spec-outdated` — the revision (stored, or carried in `.emery/`) was written under an older grammar than the running binary reads; regenerate.
- `revision-invalid` — the carried `.emery/spec.json` / `.emery/design.json` pair is incomplete, not `emery show --format json` envelopes, or not a revision; restore both from one revision or remove `.emery/`.
