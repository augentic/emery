...
Stream roles are part of the contract: the semantic result body (text or JSON) is stdout; the failure body and live tracing are stderr. The failure body carries no styling in either format; colour policy belongs to the tracing layer alone. Tracing follows the process `RUST_LOG`. The shipped runtime defaults every guest's `RUST_LOG` to `emery_sdk=info` when the process sets none, so on a bare run the SDK reports each model turn as it opens and a turn retried or failed at WARN, and the engine stays at omnia's `error` default. An operator's own `RUST_LOG` replaces that default whole, for the engine and every adapter: `RUST_LOG=info emery specify …` admits the engine's progress, `RUST_LOG=debug` adds its DEBUG detail and the SDK's, `RUST_LOG=emery_sdk=debug` admits the SDK alone (what each turn yielded, each reference-tool call, and each rejected candidate), and `RUST_LOG=off` silences every guest. The host never rewrites the process environment. A failure's description is printed once, in the result body; tracing names the failed step and its class, never the text. Skills follow the plugin rule's tracing contract and relay the semantic result once without repeating tracing lines.
