...
Stream roles are part of the contract: the semantic result body (text or JSON) is stdout; the failure body and live tracing are stderr. The failure body carries no styling in either format; colour policy belongs to the tracing layer alone. Tracing verbosity is selected by the global flags `-v` / `--verbose` (counted) and `-q` / `--quiet`, part of the `emery` grammar and accepted anywhere in argv: a bare invocation shows INFO progress, `-v` adds the engine's DEBUG detail, `-vv` adds its TRACE detail and every engine-guest crate's DEBUG, and `-q` turns the engine's tracing off. An ambient `RUST_LOG` refines every level, `-q` included — its valid directives apply after the flag's, so for a target both name the environment's wins and a bare level in it replaces the flag's; an invalid directive is diagnosed on stderr and ignored. Combining `-v` with `-q` is a usage error (exit `64`). The flags reach every adapter the run dispatches as a level, not as directives: `info` on a bare run (the SDK reports each model turn as it opens, and a turn retried or failed at WARN), `off` under `-q`, and under `-v` the DEBUG of the adapter's own crate, its SDK (what each turn yielded, each reference-tool call, and each rejected candidate), and `omnia_sdk`. The process `RUST_LOG` refines each guest's filter independently after its preset, and the host never rewrites it. A failure's description is printed once, in the result body; tracing names the failed step and its class, never the text. Skills follow the plugin rule's tracing contract and relay the semantic result once without repeating tracing lines.
