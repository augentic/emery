...
// One entry of the generated table: a document's tree-relative path and the
// canonical file the table `include_str!`s.
struct Entry {
    path: String,
    file: PathBuf,
}

// The generated file names `Doc` unqualified; the including macro brings it
// into scope through `$crate`, so the generated code never spells the crate
// name and survives a renamed dependency.
const HEADER: &str = "// @generated by emery-prose — do not edit
...
// Walks `root` and writes the sorted table and its `docs()` accessor to
// `out_dir/prose_docs.rs`.
...
fn walk(dir: &Path, path: &str, files: &mut Vec<Entry>, stack: &mut Vec<PathBuf>) -> Result<()> {
...
fn walk_entries(
    dir: &Path, path: &str, files: &mut Vec<Entry>, stack: &mut Vec<PathBuf>,
) -> Result<()> {
...
mod tests {
    ...
    fn embeds() {
        ...
        assert!(generated.contains("pub fn docs() -> &'static [Doc]"), "{generated}");
    }

    // Keep (entry-point-unreachable): a dangling link and a symlink cycle each
    // fail the build; no live corpus can arrange either.
    #[test]
    fn refuses() {
        let tmp = tempfile::tempdir().expect("tempdir");
        let out = tmp.path().join("out");
