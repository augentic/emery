...
Your `preamble` opens the document: a short overview of what was bound and what the requirements say — every requirement, the unchanged ones included — as paragraphs. One flat document: no delta sections, no per-domain splits — `spec.md` is the whole reviewable set. Requirements are rendered in id order, whatever order the request lists them; only the listed subjects are drafted, and the rest carry their scenarios over from the revision this run continues.
