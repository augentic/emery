...
| `crates/sdk` | The guest-only adapter SDK: `SourceAdapter` with its provided `extract` over an adapter's `survey`, the `survey` helpers (the walk, the cut by directory, the one-call cut by model), and the `source!` export macro. No production crate depends on it |
