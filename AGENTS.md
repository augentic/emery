...
| `src/` | `lib.rs`: the wasm32 engine guest. `main.rs`: the shipped runtime — `mod host { omnia::runtime!({..}) }` holds the compiled-in policy (the invocation directory mounts read-only as `.`, revision state lives in `.omnia/storage`, Cursor answers the model) and a hand-written `main` plans each run's deployment over it from `emery_cli::plan(argv)`: every local `.wasm` path and package the run names is declared as an on-demand guest before the engine runs — a component under its file's stem, a package under its exact reference, each pinned by its `[[source]] digest` — the project's `emery.toml` `[registries]` table is installed as the deployment's routing (`emery` is `augentic.io` unless a line re-routes it), and one tracing level governs the host and every guest — `info` bare, one step up per `-v` and down per `-q`, a process `RUST_LOG` refining a bare run — from the flags the grammar declares through `omnia_sdk::api::command::Verbosity` |
...
- Every adapter reference goes through the `omnia:plugins/loader` capability by the guest name it derives (`AdapterRef::guest`), and the deployment's guest list is the allow-list: a project-relative `.wasm` path or an exact package reference loads fresh on every run as the on-demand guest the runtime declares for it before the engine runs (a `[[source]] digest` is its pin, declared on the entry and held by the engine against the digest the loader attests); a bare name is a guest the deployment declares at boot — the loader attests it or refuses it typed, so an undeclared name never reaches a dispatch trap. Two references that name one guest (two components sharing a file stem) are refused before any load. GitHub URLs are refused. The only pre-compiled artifact is the engine compiled into the binary; adapters are always raw wasm — every entry the runtime declares from an invocation is `wasm_only`, so a pre-compiled artifact under an adapter's path is refused typed however it hashes, since the path, its pin, and its registry routing come from the same project the run is asked to trust.
