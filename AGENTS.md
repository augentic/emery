...
| `src/` | `lib.rs`: the wasm32 engine guest. `main.rs`: the shipped runtime — one `omnia::runtime!` block; the invocation directory mounts read-only as `.`, revision state lives in `.omnia/storage`, Cursor answers the model, adapters load from local `.wasm` paths under that mount (registered under the file's stem) or from the registry the project's `emery.toml` `[registries]` table routes their namespace to (`emery` is `augentic.io` unless a line re-routes it; the deployment declares no routing of its own), and one tracing level governs the host and every guest — `info` bare, one step up per `-v` and down per `-q`, a process `RUST_LOG` refining a bare run — set by omnia from the flags the grammar declares through `omnia_sdk::api::command::Verbosity` |
...
- Every adapter reference goes through the `omnia:plugins/loader` capability: a project-relative `.wasm` path or an exact package reference loads fresh on every run (a `[[source]] digest` is its pin); a bare name is a guest the deployment declares — the loader attests it or refuses it typed, so an undeclared name never reaches a dispatch trap. Two references that would register under one name (two components sharing a file stem) are refused before any load. GitHub URLs are refused.
